...
@Component({
  ...
  imports: [SelectPrimaryComponent, ButtonComponent, BackButtonComponent, CommonModule, FormsModule],
...
generateReport() {
  this.transactionService.getAll().subscribe(transactions => {
    this.transactions = transactions.filter(t => {
      ...
      return matchesCustomer && matchesAccount && matchesStartDate && matchesEndDate;
    });
